...
export function promiseTask(want: Want) {
  return want === 'Security' ? 'Deliver a paying sponsor campaign' : want === 'Visibility' ? 'Complete media work or a sponsor campaign' : 'Complete a stylist, social media or press project';
}
export function nextClientStep(s: State, p: Athlete) {
  ...
  if (training) return { title: 'Preparation in progress', detail: `${Math.max(0,training.due-s.week)} weeks until the report. Advance the week to finish the work.`, tab: 'Brand Studio', action: 'View preparation' };
  if (!p.delivered) return { title: 'Deliver your promise', detail: promiseTask(p.promise) + '.', tab: p.promise === 'Security' ? 'Deals' : 'Brand Studio', action: p.promise === 'Security' ? 'Find a sponsor deal' : 'Prepare for deals' };
...
export function ClientJourney({s,p,go}:{s:State;p:Athlete;go:(tab:string)=>void}) {
  ...
  return <section className="as-panel as-client-journey" aria-label={`${p.name} career path`}>
    <div className="as-career-track" aria-label="Career path"><span>✓ Sign client</span><span aria-current={s.week < 12 ? 'step' : undefined}>Prepare → Deal → Payday</span><span aria-current={s.week >= 12 ? 'step' : undefined}>Renew / move / draft</span></div>
